...
<template>
  <div class="overflow-hidden border-b border-gray-100 bg-white last:border-b-0">
    <!-- Header/Trigger -->
    <div
    ...
    >
      <slot name="header" :is-expanded="isExpanded" :toggle="toggleExpand">
        ...
        <div class="flex min-w-0 flex-1 items-center gap-4">
          ...
          />
          <div class="min-w-0">
            <h4 class="mb-1 leading-none font-bold text-gray-900">{{ transaction.description }}</h4>
            <p class="truncate text-xs text-gray-500">{{ transaction.groupName }}</p>
          </div>
        </div>
      </slot>
    ...
    <Dialog v-model:open="showDeleteConfirm">
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete "{{ transaction.description }}"? This will remove it
            from group balances and record the deletion in activity.
          </DialogDescription>
        </DialogHeader>
    ...
    >
      <div class="overflow-hidden">
        <TransactionDetailContent :transaction="transaction" :current-user-id="currentUserId" />
      </div>
    </div>
  </div>
</template>
