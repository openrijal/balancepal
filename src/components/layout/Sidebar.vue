...
const currentPath = computed(
  () =>
    props.currentPath || (typeof window !== 'undefined' ? window.location.pathname : '/dashboard')
);
...
<template>
  ...
  <!-- Desktop Sidebar -->
  <div v-else class="custom-scrollbar flex h-full flex-col overflow-y-auto bg-white">
    <!-- Logo -->
    <div class="mb-2 p-4">
      <a href="/dashboard" class="flex items-center gap-2">
        <div class="bg-primary-500 flex h-8 w-8 items-center justify-center rounded-lg shadow-sm">
          <span class="text-sm leading-none font-bold text-white">B</span>
        </div>
        <span class="text-lg font-bold tracking-tight text-gray-900">Balance Pal</span>
      </a>
    </div>
    ...
    <div class="mb-6 px-2">
      ...
      <div class="space-y-0.5">
        ...
        <p v-if="groups.length === 0" class="px-3 py-2 text-[11px] text-gray-400 italic">
          No groups yet
        </p>
      </div>
    </div>
    ...
    <div class="mb-6 px-2">
      ...
      <div class="space-y-0.5">
        ...
        <p v-if="friends.length === 0" class="px-3 py-2 text-[11px] text-gray-400 italic">
          No friends yet
        </p>
      </div>
    </div>
  ...
  <Dialog v-model:open="createGroupOpen">
    <DialogContent class="sm:max-w-[425px]">
      ...
      <form class="grid gap-4 py-4" @submit.prevent="handleCreateGroup">
